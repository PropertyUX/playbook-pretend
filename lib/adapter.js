 ...
 * @return PretendAdapter    A new mock adapter
 */
class PretendAdapter extends Adapter {
  constructor (robot) {
    super(robot)
    _this = this
    this.name = 'pretend'
  ...
  */
   ...
   *
   * @param  {Object} envelope   Message, room and user details
   * @param  {array}  strings... One or more Strings for each message to send
   */
  ...
  /**
   * Custom method used by some platform adapters to send direct to a user.
   *
   * @param  {Object} envelope   Message, room and user details
   * @param  {array}  strings... One or more Strings for each message to send
   */
  sendDirect (envelope, ...strings) {
    envelope.room = `direct_${envelope.user.id}`
  ...
  */
  ...
  response (user, text) {
    ...
    return new Response(this.robot, message, match)
  }
}

module.exports = {
  use: function (robot) {
    return new PretendAdapter(robot)
  }
}
